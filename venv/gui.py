...
def predict():
    ...
    model = tf.keras.models.load_model("/Users/sahitjain/PycharmProjects/sunhacks2019/CNN3.model")
    data_path = filename
    image = prepare(data_path)
    prediction = model.predict([image])
    prediction = list(prediction[0])
    print(cat[prediction.index(max(prediction))])
    return(cat[prediction.index(max(prediction))])
...
ttk.Label(mainframe, textvariable=Accuracy).grid(column=3, row=3, sticky=(W, E))
